...
def make_seesaw_spec_fn() -> SceneSpecFn:
    """Create a damped, range-limited seesaw with a fixed counterweight."""

    def add_seesaw(spec: MjSpec) -> None:
...
def _add_fulcrum(spec: "MjSpec") -> None:
    ...
    body = spec.worldbody.add_body(name="seesaw_fulcrum")
    body.pos = (x, y, half_height)
    body.add_geom(
        name="seesaw_fulcrum_base",
        type=MJGEOM_BOX,
...
def _add_seesaw(spec: "MjSpec") -> None:
    body = spec.worldbody.add_body(name="seesaw_plank")
    body.pos = PIVOT
    body.add_joint(
        name="seesaw_hinge",
        type=MJ_JOINT_HINGE,
